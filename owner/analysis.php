...
<head>
    ...
    <title>CR Owner - Dashboard</title>
...
$rents =query('SELECT * FROM `owner_booking` WHERE owner_id='.user()['id']);
$rent_dates=array();
$rentcounts=array();
...
for($from=strtotime($dates[0]['from_date']);$from<=strtotime($dates[0]['to_date']);$from=strtotime('+1 day',$from)){
    ...
    $rentcounts[]=count($rents);
}

// print_r(json_encode($rentcounts));

$categories = query('SELECT COUNT(owner_vehicle.owner_brand_id) AS categorycount, owner_brand.name FROM `owner_vehicle` JOIN owner_brand ON owner_vehicle.owner_brand_id = owner_brand.id WHERE owner_vehicle.user_id = '.user()['id'].' GROUP BY owner_brand_id');



$vechiles = query('SELECT vehicle_id, COUNT(vehicle_id) as vechile_count, owner_vehicle.name FROM `owner_booking` JOIN owner_vehicle ON owner_vehicle.id=owner_booking.vehicle_id WHERE owner_vehicle.user_id = '.user()['id'].' GROUP BY vehicle_id');
