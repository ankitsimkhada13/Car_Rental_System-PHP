...
$filter = request('sort_numeric');

if(isset($filter)){
    $result = query("SELECT * FROM `vehicles` ORDER BY `price` $filter ");
$result1 = query("SELECT * FROM `owner_vehicle` ORDER BY `price` $filter ");
}
...
<section id="contact" class="contact">
    ...
    <form class="d-flex mt-3 float-start mx-3"  style="max-width: 160px;margin:auto" action="car_listing.php" method="GET">
        <select name="sort_numeric" class="form-select">
                                            <option value="">Filter</option>
                                            <option value="asc" <?php if(isset($_GET['sort_numeric']) && $_GET['sort_numeric'] == "low-high") { echo "selected"; } ?> > Low - High</option>
                                            <option value="desc" <?php if(isset($_GET['sort_numeric']) && $_GET['sort_numeric'] == "high-low") { echo "selected"; } ?> > High - Low</option>
                                        </select>
